...
- `Abenteuer 5 - Weggabelung` – Planung Abenteuer 5 (jetzt linear: Myravel/Webway)
...
- `image_prompt` – Midjourney-V8.1-Arbeitsanweisung (Fotorealismus) + fertige NPC-/Orts-Prompts (Abenteuer 5: Hauptszenen & Basar; Abenteuer 6: Katharina Sahl als Seraphim)
...
- `stl/_compress_z.py` – nichtlineare Z-Stauchung der Reliefspitzen (Soft-Knee/tanh gegen eine Decke, Vertex-basiert, boolean-frei). Erzeugt `*_soft.stl`. Aufruf: `python other_media/stl/_compress_z.py [datei.stl] [--knee H] [--ceiling H]`; ohne Argumente alle `*_cut2.stl` mit Auto-Parametern (Knie = P75, Ceiling = Knie + max(0,8, 15 % Resthöhe)). Hinweis: global-monotone Stauchung bügelt Detail auf hohen Sockeln weg – dafür besser `_relief_opt.py` nutzen.
- `stl/_relief_opt.py` – **adaptive Relief-Optimierung** (Bas-Relief-Prinzip): Median-Basis/Detail-Zerlegung, Basis-Remap auf gemeinsames Plateau (hebt mittlere Sockel an, senkt hohe ab), lokale Detail-Normalisierung auf einheitliche Amplitude; PNG-Vorlagen (`<gott>.png`) für Orientierungs-Check und optionales Bilddetail (`--img-detail`). Das Ergebnis wird als **Heightfield-Neuaufbau** erzeugt (nur die Draufsicht zählt, Meshy-Überhänge werden verworfen): wasserdichtes Mesh aus transformiertem Höhenfeld (`--grid`, Default 1024) + senkrechte Außenwand + planer Boden; stetige Rand-Anhebung aufs Plateau (`--rim-mm`). Modi: `raise` (Detail erhaben, Default) / `inset` (Detail ins Plateau eingelassen, max. Auflagefläche). Erzeugt `*_opt.stl` / `*_opt-inset.stl` + `*_preview.png` (vorher|nachher). Braucht numpy/scipy/pillow (im `.venv`).
- `stl/_side_view.py` – **Seitenansicht-Kontrolle**: rendert orthografische Seitenansichten + Querschnitte mit roter Referenzlinie auf Maximalhöhe (`*_side.png`) sowie Schrägsichten 22,5°/45° aus vier Richtungen als Einzeldateien (`*_ob_az<phi>_e<elev>.png`), zur Selbstkontrolle der Skript-Ergebnisse ohne Slicer. Aufruf: `python other_media/stl/_side_view.py datei.stl [--ppm 8] [--zx 2]`.
