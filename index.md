...
- `stl/_relief_opt.py` – **adaptive Relief-Optimierung** (Bas-Relief-Prinzip): Median-Basis/Detail-Zerlegung, Basis-Remap auf gemeinsames Plateau (hebt mittlere Sockel an, senkt hohe ab), lokale Detail-Normalisierung auf einheitliche Amplitude; PNG-Vorlagen (`<gott>.png`) für Orientierungs-Check und optionales Bilddetail (`--img-detail`). Die Vertex-Transformation ist **vertex-relativ über ausschließlich glatte Felder** (`z' = f(Basis) + (z−Basis)·Skala`) – verrauschte Per-Zellen-Versätze erzeugten früher Zacken-Artefakte. Modi: `raise` (Detail erhaben, Default) / `inset` (Detail ins Plateau eingelassen, max. Auflagefläche). Erzeugt `*_opt.stl` / `*_opt-inset.stl` + `*_preview.png` (vorher|nachher). Braucht numpy/scipy/pillow (im `.venv`).
- `stl/_side_view.py` – **Seitenansicht-Kontrolle**: rendert orthografische Seitenansichten + Querschnitte mit roter Referenzlinie auf Maximalhöhe (`*_side.png`) sowie Schrägsichten 22,5°/45° aus vier Richtungen (`*_oblique.png`), zur Selbstkontrolle der Skript-Ergebnisse ohne Slicer. Aufruf: `python other_media/stl/_side_view.py datei.stl [--ppm 8] [--zx 2]`.
