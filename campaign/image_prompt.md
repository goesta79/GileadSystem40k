...
| `--hd` | Flag (1.33 GPU-Min) | Native 2048px – **ersetzt den fehlenden Upscaler** |
| `--sd` | Flag (<1 GPU-Min) | günstige Variante zum Testen |
| `--draft` / ⚡ | Flag | 24 günstige Test-Bilder, gute dann via *Vary* hochziehen |
| `--ar` | Porträt `4:5`/`2:3`, Szene `16:9`/`21:9` | Seitenverhältnis |
...
- **Porträt:** `--ar 4:5 --style raw --s 60 --hd --v 8.1 --no anime, cartoon, illustration, painting, cgi, 3d render, plastic skin, smooth airbrushed skin, deformed hands`
- **Szene/Ort:** `--ar 16:9 --style raw --s 80 --hd --v 8.1 --no anime, cartoon, illustration, painting, cgi, 3d render`
...
3. **Erst `--draft`/`--sd`** zum Finden der Komposition, dann beste Variante mit **`--hd`** final rendern.
